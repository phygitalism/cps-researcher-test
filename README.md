...
Необходимые навики и требования:
- Знание современных библиотек глубокого обучения: TensorFlow, PyTorch, CNTK и пр.
- Знание основ компьютерной графики: способы представления и преобразование 3D моделей, знание математических основ, алгоритмов визуализаций.
- Знание современных подходов к решению задач классификации, кластеризации, порождения. Особенно важны подходы для работы с 3D данными (меш, облако точек, воксели, функции пространственной принадлежности).
- Опыт работы на проекте, в котором присутствовали 3D данные для анализа.
- Умение развёртывать весь pipline по работе с данными.

Далее представлены два задания (первое для junior, второе для middle-senior). В каждом этапе присутсвую этапы и подпункты. Чем болььше выполнено этапов и подпунктах в них - тем лучше.

## Camera transformation task (jun)

В файле 'Test_Mesh.py' можно найти вершины и нормали некотороего объекта в канонической позиции. Требуется реализовать на каком-либо языке программирования (предпочтительтно Python) алгоритм оценивания произведённой трансформации (вращения(кватернион) + перемещения(вектор)) над объектом, который находился до преобразования в канонической позиции.

Для этого:
- Реализовать генератор пар объектов "преобразование - координаты вершин и нормалей объекта после преобразования".
- Реализовать визуализацию объекта и трансформации в каком либо виде.
- Задачу воостановления преобразования по имеющемуся преобразованому объекту решить "классическим" методом, минимизация невязки для решения уравнения трансформации.
- Задачу восстановления решить с помощью аппроксимацией нейронной сетью, использую генератор пар объектов для создания датасэта.

## Point cloud analisys (mid-sen)
